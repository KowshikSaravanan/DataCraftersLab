...
def fetch_silver_layer_data():
    conn = get_connection()
    query = "SELECT * FROM stock_data_silver"
...
def predict_future_data(**kwargs):
    # Load the data
    df = kwargs['ti'].xcom_pull(task_ids='fetch_silver_layer_data')
    ...
    for symbol in df['symbol'].unique():
        ...
        symbol_df = symbol_df[['close']].dropna()
...
def save_predictions_to_postgres(**kwargs):
    ...
    create_table_query = """
    CREATE TABLE IF NOT EXISTS future_predictions_close (
        date DATE NOT NULL,
    ...
    truncate_table_query = """ truncate table future_predictions_close """
    cursor.execute(truncate_table_query)

    for symbol in pd.read_sql("SELECT DISTINCT symbol FROM stock_data_silver", conn)['symbol']:
        future_df = kwargs['ti'].xcom_pull(key=f'{symbol}_future_predictions', task_ids='predict_future_data')

        for _, row in future_df.iterrows():
            insert_query = sql.SQL("""
                INSERT INTO future_predictions_close (date, symbol, predicted_close)
                VALUES (%s, %s, %s)
            """)
...
with DAG(
    ...
    description='Predict future stock data from the silver layer',
    ...
    fetch_data = PythonOperator(
        task_id='fetch_silver_layer_data',
        python_callable=fetch_silver_layer_data,
    )
